...
partner_choice <- bind_rows(husbands, wives) |>
  mutate(race_spouse=factor(race_spouse, 
                            levels=c("White","Black","Indigenous","Asian",
                                     ...
                                     "Black/Indigenous", "Indigenous/Asian")))
...
partner_choice |>
  ...
  mutate(prop = n/total,
         ...
         overlap = case_when(
           race_ego == race_spouse ~ "Exact overlap (endogamy)",
  ...
  coord_flip()+
  labs(x="race of other spouse", y="percent", fill = "identification overlap")+
  theme_bw()
```
