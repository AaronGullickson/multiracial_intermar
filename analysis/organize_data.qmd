...
# convenience function for getting all the single people
...
```{r}
acs <- acs |>
  select(year, mar_market, marst, dur_mar, 
         id, sex, age, race, educ, lang, bpl, yr_usa,
         id_sp, sex_sp, age_sp, race_sp, educ_sp, lang_sp, bpl_sp, yr_usa_sp)
```
...
* Married individuals in same-sex relationships
* Cohabitors - these are identified by a valid spouse id but not married, spouse present
...
acs <- acs |>
  filter((is.na(yr_usa) | yr_usa>years_mar) & 
           ...
           ((marst!="Married, spouse present" & 
               marst!="Married, spouse absent") | dur_mar<=years_mar) &
           !(!is.na(sex_sp) & sex==sex_sp) &
           !(marst!="Married, spouse present" & !is.na(id_sp)))

# checks
summary(acs$yr_usa)
summary(acs$yr_usa_sp)
tapply(acs$dur_mar, acs$marst, max)
table(acs$sex, acs$sex_sp, exclude=NULL)
acs |> 
  group_by(marst, !is.na(id_sp)) |>
  summarize(n=n())
```
...
unions <- unions |>
  select(mar_market, id, age, race, educ, lang, bpl, id_sp, race_sp, educ_sp, 
         lang_sp, bpl_sp) |>
  rename(idh=id, ageh=age, raceh=race, educh=educ, langh=lang, bplh=bpl,
         idw=id_sp, agew=age_sp, racew=race_sp, educw=educ_sp, langw=lang_sp, bplw=bpl_sp) 

# create alternate partner data
female_alt <- acs |>
  filter(sex=="Female") |>
  select(mar_market, id, age, race, educ, lang, bpl) |>
  rename(idw=id, ageh=age, racew=race, educw=educ, langw=lang, bplw=bpl)

male_alt <- acs |>
  filter(sex=="Male") |>
  select(mar_market, id, age, race, educ, lang, bpl) |>
  rename(idh=id, agew=age, raceh=race, educh=educ, langh=lang, bplh=bpl)
```

```{r}
#| label: create-fake-unions

generateCouples(5, unions, male_alt, female_alt, geo="mar_market", verbose=TRUE)
