...
I need to decide my criteria for a recent marriage. I set this parameter for the limit on marriage duration right at the top of this document so that it is easy to change and adjust later..
...
Now, I am ready to read the data in. I can use some nice features of `readr` to read all of the datasets in with a single call to `read_fwf`.
...
Now, that I have these datasets, I can feed them into the `generateCouples` function to create my marriage market data. Eventually, I will replicate five separate marriage market datasets to handle potential variability in randomizing fake unions, but for now I am just loading one.
...
Lets do a summary on this just to make sure it all looks good.
...
```{r}
#| label: save-data
save(mar_market, file=here("data","data_constructed","markets.RData"))
```