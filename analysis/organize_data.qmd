...
I create the starting and ending indices as well as the variable names for what I will pull from the fixed-width data below. These numbers should correspond to those above. I capitalize all variables from the original data to distinguish my constructed variables (always lower case) from variables directly in the original data.
...
# check age when migrated to the US
summary(acs$age_usa)
...
unions <- unions |>
  select(mar_market, id, age, race, educ, lang, bpl, id_sp, age_sp, race_sp, 
         educ_sp, lang_sp, bpl_sp) |>
...
mar_market <- generateCouples(5, as.data.frame(unions), as.data.frame(male_alt), 
                              as.data.frame(female_alt), geo="mar_market", 
                              verbose=TRUE)

mar_market <- as_tibble(mar_market)
```
