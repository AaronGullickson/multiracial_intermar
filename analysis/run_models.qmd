...
add_vars <- function(market) {
  ...
  # simplify Latino for race_exog_full and create a simple race_exog with
  # Latino also collapsed
  market$race_exog_full <- as.character(market$race_exog_full)
  market$race_exog_full <- str_replace(market$race_exog_full, 
                                       "Latino, Single Race|Latino, Multiple Race", 
                                       "Latino")
  market$race_exog_full <- relevel(factor(market$race_exog_full), 
                                   "Endog")
  
  market$race_exog_lsplit <- market$race_exog
  market$race_exog <- as.character(market$race_exog)
  market$race_exog <- str_replace(market$race_exog, 
                                       "Latino, Single Race|Latino, Multiple Race", 
                                       "Latino")
  market$race_exog <- relevel(factor(market$race_exog), 
                              "Endog")
  ...
  market$race_exog_asym <- str_replace(market$race_exog_asym, 
                                       "Latino, Single Race|Latino, Multiple Race", 
                                       "Latino")
...
# constrained racial exogamy, with latinos split
formula_short_sym_lsplit <- update(formula_base, .~.+
                                     race_exog_lsplit+multi_shared_ancestry)
...
model_short_sym <- poolChoiceModel(formula_short_sym, data=mar_markets, 
                                   method="efron")
model_short_sym_lsplit <- poolChoiceModel(formula_short_sym_lsplit, 
                                          data=mar_markets, method="efron")
...
```{r}
#| label: save-output
save(model_full_sym, model_short_sym, model_short_sym_lsplit, model_short_asym,
