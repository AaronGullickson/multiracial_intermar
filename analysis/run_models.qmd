...
add_vars <- function(market) {
  ...
  # now collapse multiracial/multiracial cases to two cases:
  #  - multi/multi, no shared ancestry
  #  - multi/multi, shared ancestry
  ...
  market$race_exog_short_sym <- ifelse(market$race_exog_short_sym=="Multi/Multi, no shared ancestry" & 
                                         (str_detect(constituent2, constituent1.1) | 
                                            str_detect(constituent2, constituent1.2)), 
                                       "Multi/Multi, shared ancestry",
                                       market$race_exog_short_sym)
  ...
  market$race_exog_short_asym <- createExogamyTerms(market$raceh, market$racew, 
                                                    symmetric=FALSE)
  # replace with multi/multi coding where needed
  market$race_exog_short_asym <- ifelse(str_count(market$race_exog_short_asym, "-")==2,
                                        as.character(market$race_exog_short_sym), 
...
mar_markets[[1]] |>
  ...
  summarize(prop_noshared=mean(race_exog_short_sym=="Multi/Multi, no shared ancestry"),
            prop_shared=mean(race_exog_short_sym=="Multi/Multi, shared ancestry"))
