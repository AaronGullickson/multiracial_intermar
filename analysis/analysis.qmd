...
```{r}
#| label: tbl-market-type
#| tbl-cap: Distribution of marriage market type for all alternate partners

male_alt |>
  bind_rows(female_alt) |>
  mutate(market_type = factor(str_sub(mar_market, 1, 1),
                              levels = c("M", "C", "P"),
                              labels = c("Metro Area", "County", "PUMA"))) |>
  group_by(market_type) |>
  summarize(n = n()) |>
  ungroup() |>
  mutate(prop = n / sum(n)) |>
  gt() |>
  cols_label(market_type ~ "Type of marriage market",
             n ~ "N",
             prop ~ "Percent") |>
...
partner_choice |>
  ...
  mutate(race_ego=factor(race_ego, 
                         levels=c("White-Asian","White-Indigenous","White-Black",
                                  "Asian-Indigenous","Black-Asian","Black-Indigenous"))) |>
...
```{r}
#| label: fig-megaplot
#| fig-cap: Odds of interracial marriage across all cases from the full model. 83.4% and 95% confidence intervals are shown for each estimate with thick and thin bars, respectively.
#| fig-height: 10
...
#| tbl-cap: Model predicting union formation
#| results: asis
