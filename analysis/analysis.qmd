...
multi_noshare <- exp(model_short_sym$coefficients["race_exog_short_symMulti/Multi",1])
multi_share <- multi_noshare*
  exp(model_short_sym$coefficients["multi_shared_ancestryTRUE",1])
```
...
@fig-odds-ratios-mult below shows the odds ratios of outmarriage to each other multiracial group for each ego multiracial group, separately by panel.
...
means <- tibble(value=c(multi_share, multi_noshare), overlap=c(TRUE, FALSE))
...
coef_full |>
  ...
  geom_hline(data=means, aes(yintercept=value, color=overlap), linetype=2)+
  geom_linerange(linewidth=1.25, 
                 aes(ymax=exp(coef+1.386*se), ymin=exp(coef-1.386*se)))+
  geom_linerange(linewidth=0.5, 
                 aes(ymax=exp(coef+1.96*se), ymin=exp(coef-1.96*se)))+
...
coef_short_sym |>
  ...
  geom_hline(yintercept=multi_noshare, color="orange", linetype=2)+
  geom_hline(yintercept=multi_share, color="royalblue", linetype=2)+
  geom_linerange(linewidth=1.25, 
