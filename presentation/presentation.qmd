...
exogamy_trend |>
  ...
  labs(y = "Percent of married couples\nthat are of mixed race",
...
- Discuss three generation case of White/Asian Mixing

## Two-Group Simulation Can Produce Surprising Results {.smaller}
...
plot_sim <- total_pop_group |>
  ...
  scale_y_continuous(labels = scales::percent)+
  labs(y = NULL,
       caption = "Simulation assumes high rate of intermarriage, but decline in multiracial classification in later generations")

plot_sim
```

## Two-Group Simulation Can Produce Surprising Results
...
In this case, we would expect that multiracial individuals have higher likelihood of marriage with one another, irrespective of ancestry combination. 
...
We might also expect a *transcendent pattern* in which multiracial individuals are more likely to marry individuals of any racial identification (single or multiple), consistent with a "post-racial" vision of multiraciality.
...
## Simplified Multiracial-to-Multiracial Parameters
