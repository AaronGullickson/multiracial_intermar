...
- Check out the paper now out in *Demography*!
:::
...
```{r}
#| label: generate-article-qr

generate_svg(qr_code("https://doi.org/10.1215/00703370-12530612"),
             foreground = "#2C5D63",
             background = "#F4F4F2",
             filename = here("presentation", "article_qr.svg"))
```

![Article Link](article_qr.svg)
...
```{r}
#| label: generate-rep-package-qr

generate_svg(qr_code("https://github.com/AaronGullickson/multiracial_intermar"),
             foreground = "#1F4E79",
             background = "#F4F4F2",
             filename = here("presentation", "rep_package_qr.svg"))
```

![Replication Package](rep_package_qr.svg)

:::

::::

## Latino Patterns of Intermarriage {visibility="uncounted"}
...
## Gender-Specific Patterns {visibility="uncounted"}

```{r}
#| label: gender-pattern-fig
